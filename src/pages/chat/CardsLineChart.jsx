...
function CardsLinechart({ alltransactions, selectedPeriod ,datatype}) {
  ...
  useEffect(() => {
    ...
    if (selectedPeriod === "Day") {
      ...
      processedData = Object.keys(visitsByHour).map((hour) => ({
        ...
        Customers: visitsByHour[hour], // Total visits for that hour
      }));
    } else {
      const visitsByDate = {};
      ...
      processedData = Object.keys(visitsByDate).map((date) => ({
        ...
        Customers: visitsByDate[date], // Total visits for that day
      }));
    }
