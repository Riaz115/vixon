...
function Linecharts({ alltransactions, selectedPeriod }) {
    ...
    if (selectedPeriod === "Day") {
      ...
      filteredTransactions?.forEach((transaction) => {
        ...
        if (visitsByHour[hourKey]) {
          visitsByHour[hourKey] += transaction?.Transactionamount || 0; // Aggregate visits
        } else {
          visitsByHour[hourKey] = transaction?.Transactionamount || 0; // Initialize visits
        }
      });
      ...
      processedData = Object.keys(visitsByHour).map((hour) => ({
        ...
        purchase: visitsByHour[hour], // Total visits for that hour
      }));
    } else {
      const visitsByDate = {};
      ...
      filteredTransactions?.forEach((transaction) => {
        ...
        if (visitsByDate[dateKey]) {
          visitsByDate[dateKey] += transaction?.Transactionamount || 0; // Aggregate visits
        } else {
          visitsByDate[dateKey] = transaction?.Transactionamount || 0; // Initialize visits
        }
      });
      ...
      processedData = Object.keys(visitsByDate).map((date) => ({
        ...
        purchase: visitsByDate[date], // Total visits for that day
      }));
    }
